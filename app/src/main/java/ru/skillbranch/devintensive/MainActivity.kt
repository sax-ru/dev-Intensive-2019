...
class MainActivity : AppCompatActivity(), View.OnClickListener {
     ...
     *
     * здесь задается внеший вид активности (UI) через метод setContentView().
...
//        benderImage = findViewById(R.id.iv_bender)
        ...
        val qustion = savedInstanceState?.getString("QUESTION") ?: Bender.Question.NAME.name
        benderObj = Bender(Bender.Status.valueOf(status), Bender.Question.valueOf(qustion))

        Log.d("M_MainActivity", "onCreate $status $qustion")
     ...
     *
     * Тут должен быть максимально легкий и быстрый код чтобы приложение оставалось о
     */
